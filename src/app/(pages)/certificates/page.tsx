...
export default function CertificatesPage() {
  ...
  const [activeCertificateType, setActiveCertificateType] = useState<"achievement" | "transfer" | null>(null);
  ...
  const openPreview = (type: "achievement" | "transfer") => {
    setActiveCertificateType(type);
    setIsPreviewOpen(true);
  }

  const handleFinalPrint = () => {
    if (activeCertificateType === 'achievement') {
      handlePrintAchievement();
    } else if (activeCertificateType === 'transfer') {
      handlePrintTransfer();
    }
  }
            ...
            <CardContent className="space-y-4">
              ...
              <Tabs defaultValue={activeTab} onValueChange={setActiveTab}>
                  ...
                  <TabsContent value="achievement" className="space-y-4 pt-4">
                      ...
                      <Button onClick={() => openPreview("achievement")} className="w-full">
        ...
        <DialogContent className="max-w-4xl h-[90vh]">
          ...
          <div className="h-full overflow-y-auto">
            <div className="flex justify-center scale-[0.85] origin-top">
              {activeCertificateType === 'achievement' && (
                // This div is a clone for preview, but references the original for printing
                <div className="p-8 border-8 border-primary/20 h-full relative flex flex-col justify-between bg-white text-black aspect-[sqrt(2)/1] w-full">
                  <div className="absolute inset-0 flex items-center justify-center -z-0">
                    <GraduationCap className="h-64 w-64 text-primary/5" />
                  </div>
                  <div className="z-10">
                    <header className="text-center mb-8">
                      <div className="flex items-center justify-between gap-3 mb-2">
                        {logo ? <Image src={logo} alt="Institution Logo" width={80} height={80} className="object-contain" /> : <div className="w-20 h-20"></div>}
                        <div className="flex-grow">
                          <div className="flex items-center justify-center gap-3">
                            <GraduationCap className="h-10 w-10 text-primary" />
                            <h1 className="text-3xl font-bold font-headline text-primary">VINDYAA - The Altitude of Art</h1>
                          ...
                          <p className="text-xs text-slate-600 mt-2">
                            #19, 1st Cross, 1st Main, Sri Manjunatha Layout, Basavanapura Main Rd, Near SBI Bank, Bengaluru, Karnataka 560049<br />
                            Phone: +91 95909 59005 | Email: vindyaa.art@gmail.com
                          </p>
                        </div>
                    ...
                    <div className="text-center my-10">
                      ...
                      <p className="text-base text-slate-600 mt-6">This certificate is proudly presented to</p>
                      <h2 className="text-5xl font-bold font-headline text-primary my-4">{studentName || "Student Name"}</h2>
                      <p className="text-base text-slate-600">{reason} <span className="font-bold font-headline text-primary"> {activityName || "the activity"}</span>.</p>
                    </div>
                  </div>
                  <footer className="z-10 flex justify-between items-end text-center">
                    <div>
                    ...
                    <div className="flex-shrink-0 mb-4"><Award className="h-20 w-20 text-yellow-500" /></div>
                    <div>
                      <p className="font-headline border-b-2 border-slate-400 pb-1 px-4">{signature || "Signature"}</p>
                      <p className="text-sm font-semibold text-slate-600 mt-2">{signatureTitle || "Title"}</p>
                    </div>
              ...
              {activeCertificateType === 'transfer' && (
                <div className="p-8 bg-white text-black font-serif h-full flex flex-col aspect-[1/sqrt(2)] w-full">
                  <header className="text-center mb-6">
                    <div className="flex justify-center mb-2">
                      {logo ? <Image src={logo} alt="Institution Logo" width={60} height={60} className="object-contain" /> : <div className="w-16 h-16 flex items-center justify-center"><GraduationCap className="h-12 w-12 text-black" /></div>}
                    </div>
                    <h1 className="text-3xl font-bold font-headline">VINDYAA - The Altitude of Art</h1>
                    <p className="text-xs text-slate-600 mt-1">#19, 1st Cross, 1st Main, Basavanapura Main Rd, Bengaluru, 560049</p>
                  </header>
                  <Separator />
                  <div className="text-right my-4 text-sm">Date: {new Date().toLocaleDateString('en-GB')}</div>
                  <div className="flex-grow space-y-4 text-sm leading-relaxed">
                    <h2 className="text-lg font-bold tracking-widest text-center underline">TRANSFER CERTIFICATE</h2>
                    <p className="pt-4">This is to certify that <strong>{tcStudentName}</strong>, son/daughter of <strong>{tcFatherName}</strong>, was a bonafide student of this institution from <strong>{new Date(tcAdmissionDate).toLocaleDateString('en-GB')}</strong> to <strong>{new Date(tcLeavingDate).toLocaleDateString('en-GB')}</strong>.</p>
                    <p>Details of the student as per our records are as follows:</p>
                    <table className="w-full text-left border-collapse">
                      <tbody>
                        <tr><td className="py-1 pr-2 font-semibold w-1/3">Nationality</td><td className="py-1">: {tcNationality}</td></tr>
                        <tr><td className="py-1 pr-2 font-semibold">Last Course Enrolled</td><td className="py-1">: {tcLastClass}</td></tr>
